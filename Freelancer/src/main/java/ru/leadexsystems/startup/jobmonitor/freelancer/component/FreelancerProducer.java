...
import ru.leadexsystems.startup.jobmonitor.freelancer.component.api.SearchJobs;
 ...
 */
public class FreelancerProducer extends DefaultProducer  {

    private SearchJobs searchJobs;
    private DateManager manager;

    public FreelancerProducer(Endpoint endpoint, String apiSearchUrl, int start, int limit, DateManager manager) throws NotFoundParameterException {
        super(endpoint);
    ...
    private Date getLastDate(String query) throws NotFoundParameterException {
        return manager.getLastDate(query, JobSource.freelancer);
    }

    private void setLastDate(Date date, String query) throws NotFoundParameterException {
        manager.setLastDate(date, query, JobSource.freelancer);
    }
}
